...
namespace Game
{
	class EventManager
	{
		...
		static std::vector<Hare> s_hare_list;
		static std::vector<Wolf> s_wolf_list;
	...
	public:
		...
		void setNumberOfWolf(uint32_t number);
		void setNumberOfHare(uint32_t number);
		...
		std::vector<Wolf> getWolfTeam();
		std::vector<Hare> getHareTeam();
	};
}